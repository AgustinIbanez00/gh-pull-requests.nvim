...
```lua
{
  ...
  opts = {
    ...
    diff_view = {
      ...
      commentable_zones = {
        enabled = true, -- show + markers on GitHub-valid codediff inline comment targets
        sign = "+",
        keymap = "+", -- buffer-local action to add an inline comment at the cursor
      },
      whitespace = {
      ...
      review_panel = {
        enabled = true,
        auto_open = "if_content", -- "if_content" (hunks or comments) | "never" | "always"
        ...
        close_with_dq = true,
        sections = {
          changes = true, -- show Changes folder with navigable hunks
          comments = true, -- show Comments folder with review threads
        },
        active_hunk_highlight = {
          enabled = true, -- highlight hunk node matching cursor position
          debounce_ms = 80,
        },
      },
      images = {
    ...
    queries = {
      { folder = "Pull Requests", label = "Review", query = "is:open review-requested:@me org:@org" },
      { folder = "Pull Requests", label = "Mine", query = "is:open author:@me" },
      { folder = "Pull Requests", label = "All", query = "is:open" },
    },
    ui = {
      use_neotree = true,
...
| `:GhPrCreate` / `:GhPrNew` | Open the PR creation wizard | Create a PR from a pushed head branch without CLI prompts |
...
| `:GhPrToggleReviewPanel` | Toggle unified diff review panel (hunks + comments) | Navigate changes and comments together |
...
- `:GhPrCreate` / `:GhPrNew` open the PR creation wizard.
...
- `:GhPrToggleReviewPanel` toggle the unified diff review panel (Changes + Comments).
...
`:GhPrCreate` and `:GhPrNew` open the same native Neovim wizard. It collects title, description, head branch, labels, reviewers, and draft status, then shows a final review screen before creating the PR.

- Navigation: `1`-`7` jumps to a tab and opens its editor/selector, `h`/`l` or arrows moves focus between tabs, `<CR>`/`e` opens the focused tab, and `q`/`<Esc>` cancels.
- Base branch is read-only and resolved from the repository default branch; it is always passed with `--base`.
- Head branch defaults to the current branch. Creation is blocked if head equals base or the selected branch is not available on a remote.
- Labels and reviewers are selected from preloaded GitHub options. Load failures offer retry/skip/cancel; there is no free-form label or reviewer entry.
- Final submit runs `gh pr create` non-interactively with `--title`, `--body-file -`, `--head`, `--base`, repeated `--label`, repeated `--reviewer`, and `--draft` when selected.
- Title, description, and shared comment composers show line numbers plus `Ctrl-s save | q/Esc cancel` hints.
...
| `<Plug>(gh-pr-create)` | Open PR creation wizard |
...
vim.keymap.set("n", "<leader>ghn", "<Plug>(gh-pr-create)", { remap = true, silent = true, desc = "Create PR" })
...
| `+` | Add inline comment on marked codediff commentable lines |
...
- `<localleader>c` adds an inline review comment at the current GitHub-valid diff line
- visual `<localleader>c` adds an inline review comment for a same-side selected line range (`v`/`V`)
- `<localleader>s` adds an inline suggestion comment at the current modified/right-side diff line (`suggestion` template)
- visual `<localleader>s` adds an inline suggestion comment for a modified/right-side selected line range (`v`/`V`)
...
- Top-level overview node shows active PR (`PR #N - title`) for current repository.
...
- The `PR #...` overview node renders at the same top level as `Labels`, `Files`, `Reviewers`, and the other sections, so the whole review tree is not nested under a single parent.
...
- `R` forces a refresh from GitHub for the active review PR and immediately notifies that the background refresh started.
...
- Codediff commentable `+` signs use `GhPrCommentableZone`, linked to `DiagnosticHint` by default.
